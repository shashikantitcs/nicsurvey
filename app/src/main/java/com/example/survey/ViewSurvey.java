...
import com.example.survey.Adapter.ViewSurvayAdapter;
...
public class ViewSurvey extends AppCompatActivity {
  private ViewSurvayAdapter viewSurvayAdapter;
  private ArrayList<ViewSurvayModel> viewSurvayModelArrayList = new ArrayList<>();
        ...
        public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
          ...
          if (response.isSuccessful()) {
            ...
            try {
              ...
              viewSurvayModelArrayList.clear();
              for (int i = 0; i < getstatus.length(); i++) {
                JSONObject jsonObject = getstatus.optJSONObject(i);
                ViewSurvayModel viewSurvayModel = new ViewSurvayModel();

                viewSurvayModel.setName(jsonObject.optString("name"));
                viewSurvayModel.setHousenumber(jsonObject.getString("house_no"));
                viewSurvayModel.setStatus(jsonObject.getString("status"));


                viewSurvayModelArrayList.add(viewSurvayModel);
              }
              viewSurvayAdapter = new ViewSurvayAdapter(getApplicationContext(), viewSurvayModelArrayList);
              recycler_view.setAdapter(viewSurvayAdapter);
